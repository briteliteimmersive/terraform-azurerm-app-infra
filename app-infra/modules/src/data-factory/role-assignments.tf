locals {
  role_assignment_list = flatten([
    for resource_key, resource_config in local.data_factory_configs :
    [
  ...
  role_assignments = {
    for role_assignment in local.role_assignment_list : role_assignment.role_assignment_key => role_assignment
  }
}
...
resource "azurerm_role_assignment" "role_assignment" {
  ...
  scope              = azurerm_data_factory.data_factory[each.value.resource_key].id
