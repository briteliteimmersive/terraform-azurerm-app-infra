locals {
  cosmosdb_diagnostic_settings_list = flatten([
    for cosmosdb_key, cosmosdb_config in local.cosmosdb_configs : [
      for diagnostics in cosmosdb_config.diagnostic_settings : {
        name                       = diagnostics.name
        cosmosdb_key               = cosmosdb_key
        diagnostic_key             = format("%s_%s", cosmosdb_key, diagnostics.name)
  ...
  cosmosdb_diagnostic_settings = {
    for diagnostics in local.cosmosdb_diagnostic_settings_list : diagnostics.diagnostic_key => diagnostics
  }
}

resource "azurerm_monitor_diagnostic_setting" "cosmosdb_diagnostic_settings" {
  for_each                   = local.cosmosdb_diagnostic_settings
  name                       = each.value.name
  target_resource_id         = azurerm_cosmosdb_account.cosmosdb_account[each.value.cosmosdb_key].id
