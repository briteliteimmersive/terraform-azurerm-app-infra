locals {
  mssql_diagnostic_settings_list = flatten([
    for mssql_key, mssql_config in local.sql_database_map : [
      for diagnostics in mssql_config.diagnostic_settings : {
        name                       = diagnostics.name
        mssql_key                  = mssql_key
        diagnostic_key             = format("%s_%s", mssql_key, diagnostics.name)
  ...
  mssql_diagnostic_settings = {
    for diagnostics in local.mssql_diagnostic_settings_list : diagnostics.diagnostic_key => diagnostics
  }
}

resource "azurerm_monitor_diagnostic_setting" "mssql_diagnostic_settings" {
  for_each                   = local.mssql_diagnostic_settings
  name                       = each.value.name
  target_resource_id         = azurerm_mssql_database.sql_database[each.value.mssql_key].id
